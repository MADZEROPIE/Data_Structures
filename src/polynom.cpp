...
Polynom Polynom::operator-(const Polynom& pol)//стоило помен€ть местами и всЄ заработало, маги€...
{
	Polynom res = -pol;
	return res += *this;
}
...
Polynom& Polynom::sort()
{
	Node* p1 = head;
	Node* p2 = head->pNext;
	while (p2 != head) { // ≈сли приводить подобные и сортировать за один проход, то начинаетс€ кака€-то чушь 
		while (p1->pNext != head) {
	...
	while (p2 != head) {
		while (p1->pNext != head) {
			if (p1->pNext->mon < p2->mon) std::swap(p1->pNext->mon, p2->mon);
				...
				Node* tmp = p1->pNext;
				p2->mon += tmp->mon; //Ќадо делать проверку на 0, а можно потом вызвать del_zeros, что хуже, но ...
				p1->pNext = p1->pNext->pNext;
				delete tmp;
			}
			p1=p1->pNext;
